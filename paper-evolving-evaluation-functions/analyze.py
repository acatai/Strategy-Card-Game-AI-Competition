...
def graph(paths, chunk, limit=None):
  ...
  with ExitStack() as stack:
    ...
    chunks = [chunkify(file, chunk) for file in files]
    chunks = 2 * chunks + 5 * chunks[:2]
    chunks = interleave(chunks)
...
def normalize(player):
  source = normals.match(player).group(1)
  if source == 'CoacProphet':
    return 'ProphetCoac'
  if source == 'agents':
    # return player[9:]
    return player[9:-4 if player[-4] == '-' else -2]
  if source == 'release':
    return 'Chad'
  return source
...
def score(stats, times):
  ...
  for (player1, player2), results in sorted(stats.items()):
    if player1 != player2:
      players[player1] = stats_combine(players.get(player1), results)
      score_print(f'{player1:>31} {player2:>31}', results)
  ...
  for player1, results in sorted(players.items(), key=order, reverse=True):
    ...
    score_print(f'{player1:>31}', results, f' avg={avg:6.2f}±{dev:6.2f}ms')
  ...
  playerss=(
    ...
    'Tree-frombest',
    'Tree-standard',
    'Linear-from-Linear-standard',
    'Tree-from-Linear-standard',
  )
  header = ' & '.join(f'\\multicolumn{{1}}{{c}}{{\\rotatebox{{90}}{{\\texttt{{{player}}}}}}}' for player in playerss)
  print(f'        & {header} & \\multicolumn{{1}}{{c}}{{\\rotatebox{{90}}{{\\textbf{{Global average}}}}}} \\\\')
  print('        \\midrule')
  for player1 in playerss:
    scores = ' & '.join(
      f'\\multicolumn{{1}}{{c}}{{--}}'
      if player1 == player2 else
      f'{score_count(stats[(player1, player2)])[2]:.2f}'
      for player2 in playerss
    )
    print(f'        \\hspace{{0.2cm}} \\texttt{{{player1}}} \\hspace{{0.2cm}} & {scores} & \\hspace{{0.2cm}} {score_count(players[player1])[2]:.2f} \\hspace{{0.2cm}} \\\\')
  print('        \\bottomrule')
