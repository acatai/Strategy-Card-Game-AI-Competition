...
normals = {
  "java -jar RandomWItems2lanes/agent.jar": "RandomWItems2lanes",
  "java -jar Zylo/agent.jar": "Zylo",
  "python3 ByteRL/locm1d5_submit.py": "ByteRL",
  "python3 Inspirai/agent.py": "Inspirai",
  "python3 MugenSlayerAttackOnDuraraBallV3/31072022rdycode.py": "MugenSlayerAttackOnDuraraBallV3",
  "python3 NeteaseOPD/main.py": "NeteaseOPD",
  "python3 USTC-gogogo/Zero.py": "USTC-gogogo"
}

summary = compile(r"^\d\d\d\d-\d\d-\d\dT\d\d:\d\d:\d\d '(?P<player1>.*?)' '(?P<player2>.*?)' (?P<json>{.*}) (?P<score1>-?\d) (?P<score2>-?\d)(?: cardGenSeed=-?\d+| constructedChoicesSeed=-?\d+| draftChoicesSeed=\d+| shufflePlayer0Seed=(?P<seed0>\d+)| seed=(?P<seed1>\d+)| shufflePlayer1Seed=(?P<seed2>\d+))*\s*$")
